...
def generate_query_and_response(programCode, questionTitle, questionDescription, expectedInput, expectedOutput):
    query = "Review the following code given below, written in C. But, in no way you have to give the correct answer. Don't start with something like Answer: or ChatGPT: or Solution: etc, just plain solution."\
            ...
            "\n\nProgram Code: \n\n{0}\n\n\nQuestion: \n\n{1}\n\n{2}\n\nExpected Input:\n\n{3}\n\nExpected Output:\n\n{4}".format(programCode, questionTitle, questionDescription, expectedInput, expectedOutput)
    
    response_text = response(query)
    return speaker_remove.sub("", response_text)
